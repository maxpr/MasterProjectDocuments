...
\item[$\bullet$]{Then modify the protocols' settings to run with the least significant difference in term of assumption and model.This will enable us to compare the protocols in a fair way. We compare in section 7 Prio, Unlynx and Unlynx with Prio Threat Model.}
...
Even if fewer distributed have been deployed to the real world, it is a rising and interesting subject.\\
...
This section presents the time required for the 3 different systems described in the next lines to execute a request. Figure 4  presents the protocol for each system (both Unlynx have the same pipeline). All test were run on VMWare, Ubuntu64 bits v17.10, 6.1 GB RAM, 2 cores Intel I5-4590 @3.3Ghz.\\
...
\end{figure}
First the execution times that do not change in function of the number of server (or lightly) are \textit{the Data provider proof computation} for trusted Unlynx system. It is around 50 ms, while it groes linearly in the number of server in Unlynx with 225 ms and 470 ms with 5 and 10 servers respectively.\\
The \textit{aggregation} grows with the number of server going from 10 ms at 5 servers to 45 ms at 10 servers for Unlynx. Prio starts lower at 8 ms and hits 29 ms. So Prio aggregation is more efficiently scalable with the number of servers.\\
\textit{Key Switch} also increases more than linearly when the number of server grows large, going from 69 ms to 233 ms when comparing 5 and 10 servers.\\
The most interesting result is the \textit{Proof Verification}. In Prio, it starts high (297 ms) and increases when the number of servers go larger (780ms) than 5.\\
In trusted Unlynx, it also starts high with 275 ms but decreases with the ratio $\frac{\#DP}{\#Server}$. So when there is a number of server equal or superior to the number of data provider, the time to verify is around 62 ms. Unlynx's verification phase decreases slightly with the number of server, and goes under Prio's verification time at 10 servers (629 ms vs 780 ms).\\
...
This step becomes the worst step in term of execution time, as you need to send a lot of message to all servers from all servers and wait for all to receive.
\begin{figure}[H]
\centering
...
We can state that the execution time grows linearly with circuit size, and it is mentionned in Prio's paper \cite{prio} that it is possible to do it with a sub-linear factor.\\ 
Moreover, the number of sent data is pretty low for small size input data, and grows sub linearly with the size, in bits, of the input data.\\
The size of data in the broadcast phase grows with a factor $\#servers \times \# servers-1$, which is problematic if the number of server grows high. However, the number of server in the collective authority should not grow to large, and this is still acceptable.\\


Table 2 reports the same measurements for Unlynx with the same threat model as the orginal work:\\
\begin{table}[H]
\centering
...
\end{table}
The initialization time can clearly be ignored with a really low execution time compared to the other step of the protocol.\\
The set of signatures' size is not really high, as the range should be chosen wisely, e.g, pick $16^{16}$ instead of $2^{64}$ to optimize global traffic. A major problem is the computation time for proof at DPs. It grows linearly in the number of server from 60ms to 600 ms for 10 servers. This is due to the fact that each server will send its validation to the data provider responsible for the data. It also means that the traffic for proof sending to the servers grows the same way. It reaches a size higher than $10000$ bytes for 10 servers.\\
Aggregating ciphers is a little longer than simple encoding in Prio
Both Aggregation and Key Switch are the exact same protocol as in the original work, and scale almost linearly.\\
...
\end{table}
This assumption in Threath Model allows the data provider to compute only 1 proof for 1 set of signatures, and the traffic and execution time does not scale with the number of server (60ms and 1680 bytes for higher exponent parameter).\\
As a result, the proof verification does not scale either as each server receive one proof to validate. This protocol is better than the one implemented, but sacrifices the assumption that only one server need not to be malicious, for one where the whole CA is trusted.\\

As a general conclusion for this tables, Prio's and trusted Unlynx seems to be the two protocol fitted for a real world use. Prio seems to be faster as data are not encrypted and there is no key switch protocol to perform which takes some time.
...
\section{Future Work}
Unlynx's input range validation enables to handle malicious/faulty data providers. However, the best solution implies to trust all server which is not desirable. What could be done is adapt Prio SNIPs protocol so that it can handle ciphertexts instead of simple integers.\\
Moreover, it would be possibly better than Prio as you could potentially publish the share of a cipher, the same way you publish cipher, to verify computations. This would allow keeping the assumptions about servers in Unlynx. The technical issue would be to look at if it is possible to use arithmetic circuit, MPC, and AFEs with ciphertexts. As ElGamal ciphertexts are homomorphic additive, AFE's and MPC should be doable. Points can also be serialized to an array of bytes and so potentially be taken as input of an arithmetic circuit.\\
Another alternative would be to compute the signature collectively without revealing the private key to the servers of the CA. This would either lead to research on sequential inverse addition or to new alternatives in terms of signatures functions.
...
Both Unlynx and Prio are decentralized systems for privacy-preserving data sharing among multiple data providers. They each have their own advantages and disadvantages but can be combined to have a protocol with more modularity. This project achieves an implementation of Prio in Unlynx's framework as well as a novel non-interactive input validation algorithm for Unlynx, based on Elliptic Curve pairing.\\ With the performance and scalability comparison, we can state that Unlynx input validation could be done offline and still achieve a correct execution time close to Prio' SNIP.\\
An interesting alternative would be to implement Prio SNIP validation in Unlynx's framework. The advantages of using ciphertext would allow us to keep the actual Threat Model. Such an implementation would permit input validation, but also additional aggregation functions that Prio supports.\\
However, it would scale badly when the number of server beyond 20 due to the broadcast phase of Prio, but in a context of collective authority, this number should not be attained.
\newpage
